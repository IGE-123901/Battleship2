	...
	public String processEnemyFire(boolean verbose) {
		...
		if (verbose) {
			...
			if (validShots == 0 && repeatedShots > 0) {
				output.append(repeatedShots).append(TIRO_LITERAL).append(repeatedShots > 1 ? "s" : "").append(" repetido").append(repeatedShots > 1 ? "s" : "");
			} else {
				if (validShots > 0) {
					output.append(validShots).append(TIRO_LITERAL).append(validShots > 1 ? "s" : "").append(" válido").append(validShots > 1 ? "s" : "").append(": ");
				}
				...
				if (!hitsPerBoat.isEmpty()) {
					for (Map.Entry<String, Integer> entry : hitsPerBoat.entrySet()) {
						...
						if (!sunkBoatsCount.containsKey(boatName)) {
							output.append(hits).append(TIRO_LITERAL).append(hits > 1 ? "s" : "").append(" num(a) ").append(boatName).append(" + ");
						}
					}
				}

				if (missedShots > 0) {
					output.append(missedShots).append(TIRO_LITERAL).append(missedShots > 1 ? "s" : "").append(" na água");
			...
			if (outsideShots > 0) {
				...
				output.append(outsideShots).append(TIRO_LITERAL).append(outsideShots > 1 ? "s" : "").append(" exterior").append(outsideShots > 1 ? "es" : "");
			}
